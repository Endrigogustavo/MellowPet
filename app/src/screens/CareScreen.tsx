...
function statusFor(link: CaregiverLink, loaded: Loaded | null) {
  if (!link.consent || link.consent.status !== 'active') return { text: 'aguardando permissão', color: WARN };
...
export function CareScreen() {
  ...
  const refreshCurrent = () => {
    ...
    Promise.all([fetchCareDashboardSummary(current.cared_user_id, 24 * 7), listCareAlerts(current.cared_user_id)])
      ...
      .catch((reason) => {
        setLoaded(null);
        setError(reason instanceof Error ? reason : new Error('Não foi possível atualizar os dados consentidos.'));
      });
  };
  ...
  const endCareRelationship = () => {
    ...
    Alert.alert(
      ...
      'O acesso aos dados consentidos é interrompido agora. O histórico operacional do vínculo é preservado.',
      [
  ...
  }, [current?.id, current?.cared_user_id, current?.consent?.status]);
  ...
  return <ScreenScroll>
    ...
    {linksLoading && links.length === 0 ? <Section top={0}><Card radius={24} padding={22} style={{ alignItems: 'center', gap: 9 }}><ActivityIndicator color={T.pri} /><Txt s={13} c={T.t3} accessibilityRole="progressbar" accessibilityLabel="Carregando conexões de cuidado" accessibilityLiveRegion="polite">Carregando conexões…</Txt></Card></Section> : linksError ? <Section top={0}><CareLinksErrorCard error={linksError} onRetry={refreshLinks} /></Section> : links.length === 0 ? <Section top={0}><Card radius={24} padding={22} style={{ alignItems: 'center', gap: 9 }}><Icon d={ICONS.people} size={28} color={T.t3} /><Txt s={15} w={800} c={T.t1} center accessibilityRole="summary">Você ainda não acompanha ninguém</Txt><Txt s={12.5} lh={1.55} c={T.t3} center>Gere um convite em Recursos. A pessoa precisa aceitar e definir o que deseja compartilhar.</Txt><Touchable onPress={() => actions.go('caretools')} accessibilityRole="button" accessibilityLabel="Abrir recursos de cuidador" accessibilityHint="Abre os recursos para criar um convite" style={{ marginTop: 5, paddingVertical: 10, paddingHorizontal: 16, borderRadius: 999, backgroundColor: T.priL }}><Txt s={12.5} w={800} c={T.pri}>Abrir recursos</Txt></Touchable></Card></Section> : <>
      ...
      {current ? <Section top={10}><Card radius={18} padding={14}><Txt s={12.5} w={800} c={T.t1}>Encerrar acompanhamento</Txt><Txt s={11.5} lh={1.45} c={T.t3} style={{ marginTop: 3 }}>Interrompe imediatamente seu acesso aos dados de {linkName(current)}. O histórico operacional é preservado.</Txt><Touchable onPress={endCareRelationship} disabled={revoking} accessibilityRole="button" accessibilityLabel={`Encerrar acompanhamento de ${linkName(current)}`} accessibilityHint="Interrompe seu acesso aos dados consentidos dessa pessoa" style={{ alignSelf: 'flex-start', marginTop: 10, paddingVertical: 8, paddingHorizontal: 10 }}><Txt s={11.5} w={800} c={DANGER}>{revoking ? 'Encerrando…' : 'Encerrar acompanhamento'}</Txt></Touchable></Card></Section> : null}
      {current?.consentConfigurationInactive ? <Section><CareConfigurationCard feature="O painel, os alertas e os dados consentidos" /></Section> : !current?.consent || current.consent.status !== 'active' ? <Section><Card radius={24} padding={19}><Txt s={15} w={800} c={T.t1} accessibilityRole="summary">Aguardando escolha da pessoa acompanhada</Txt><Txt s={12.5} lh={1.55} c={T.t3} style={{ marginTop: 7 }}>Ela precisa confirmar os limites de compartilhamento no próprio app. Até lá, nenhum sinal, alerta ou histórico é exibido.</Txt></Card></Section> : error instanceof CareConfigurationInactiveError ? <Section><CareConfigurationCard feature="O painel, os alertas e os dados consentidos" /></Section> : error ? <Section><Card radius={24} padding={19} style={{ borderColor: DANGER }}><Txt s={14.5} w={800} c={DANGER} accessibilityRole="alert" accessibilityLiveRegion="assertive">Não foi possível carregar os dados</Txt><Txt s={12.5} lh={1.55} c={T.t3} style={{ marginTop: 6 }}>{error.message}</Txt><Touchable onPress={refreshCurrent} accessibilityRole="button" accessibilityLabel="Tentar carregar os dados novamente" style={{ marginTop: 12 }}><Txt s={12.5} w={800} c={T.pri}>Tentar novamente</Txt></Touchable></Card></Section> : !currentData ? <Section><Card radius={24} padding={22} style={{ alignItems: 'center', gap: 9 }}><ActivityIndicator color={T.pri} /><Txt s={13} c={T.t3} accessibilityRole="progressbar" accessibilityLabel="Atualizando sinais consentidos" accessibilityLiveRegion="polite">Atualizando sinais consentidos…</Txt></Card></Section> : <>
        ...
        <Section top={4}><Txt s={11.5} lh={1.55} c={T.t3}>Os sinais apresentados são agregados e consentidos. Eles servem para orientar uma conversa, não para diagnosticar ou vigiar silenciosamente.</Txt></Section>
      </>}
    </>}
  </ScreenScroll>;
}
