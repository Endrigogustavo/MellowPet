...
export function CareAuditScreen() {
  ...
  const auditEnabled = current?.consent?.status === 'active' && current.consent.scopes.audit === true;

  const refresh = () => {
    setEntries(null);
    setError(null);
    if (!current?.cared_user_id || !auditEnabled) return;
  ...
  }, [current?.id, current?.cared_user_id, current?.consent?.status, current?.consent?.scopes.audit]);
  ...
  return <ScreenScroll>
    ...
    {linksLoading && links.length === 0 ? <Section top={0}><Card radius={24} padding={22} style={{ alignItems: 'center', gap: 9 }}><ActivityIndicator color={T.pri} /><Txt s={13} c={T.t3}>Carregando conexões…</Txt></Card></Section> : linksError ? <Section top={0}><CareLinksErrorCard error={linksError} onRetry={refreshLinks} /></Section> : !current ? <Section top={0}><Card radius={24} padding={20}><Txt s={14.5} w={800} c={T.t1}>Conecte uma pessoa para consultar o histórico.</Txt></Card></Section> : current.consentConfigurationInactive ? <Section top={0}><CareConfigurationCard feature="O histórico de cuidado" /></Section> : !auditEnabled ? <Section top={0}><Card radius={24} padding={20}><Txt s={14.5} w={800} c={T.t1}>Histórico não compartilhado</Txt><Txt s={12.5} lh={1.55} c={T.t3} style={{ marginTop: 6 }}>A pessoa acompanhada decide, em suas permissões, se o histórico de alterações importantes pode ser visto.</Txt></Card></Section> : error instanceof CareConfigurationInactiveError ? <Section top={0}><CareConfigurationCard feature="O histórico de cuidado" /></Section> : error ? <Section top={0}><Card radius={24} padding={20} style={{ borderColor: DANGER }}><Txt s={14.5} w={800} c={DANGER}>Não foi possível carregar</Txt><Txt s={12.5} lh={1.55} c={T.t3} style={{ marginTop: 6 }}>{error.message}</Txt><Touchable onPress={refresh} style={{ alignSelf: 'flex-start', marginTop: 12 }}><Txt s={12.5} w={800} c={T.pri}>Tentar novamente</Txt></Touchable></Card></Section> : entries === null ? <Section top={0}><Card radius={24} padding={22} style={{ alignItems: 'center', gap: 9 }}><ActivityIndicator color={T.pri} /><Txt s={13} c={T.t3}>Carregando histórico…</Txt></Card></Section> : <>
