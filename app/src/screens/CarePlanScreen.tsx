...
export function CarePlanScreen() {
  ...
  const refresh = () => {
    if (!state.userId || !current?.cared_user_id || current.consent?.status !== 'active') return;
    ...
    }).catch((reason) => { setConfigurationInactive(reason instanceof CareConfigurationInactiveError); setMessage(reason instanceof Error ? reason.message : 'Não foi possível carregar o plano.'); });
  };
  useEffect(() => { refresh(); /* eslint-disable-next-line react-hooks/exhaustive-deps */ }, [current?.id, current?.cared_user_id, current?.consent?.status, state.userId]);
  const save = async () => {
    ...
    try {
      ...
      refresh();
    } catch (reason) { setConfigurationInactive(reason instanceof CareConfigurationInactiveError); setMessage(reason instanceof Error ? reason.message : 'Não foi possível salvar o plano.'); }
  };
  const addNote = async () => {
    if (!state.userId || !current?.cared_user_id) return;
    if (!note.trim()) { setMessage('Escreva uma nota antes de salvá-la.'); return; }
    try { await createPrivateNote(state.userId, current.cared_user_id, note.trim()); setNote(''); setMessage('Nota privada salva.'); refresh(); } catch (reason) { setConfigurationInactive(reason instanceof CareConfigurationInactiveError); setMessage(reason instanceof Error ? reason.message : 'Não foi possível salvar a nota.'); }
  };
  ...
  return <ScreenScroll><ScreenTitle label="PLANO DE CUIDADO" title={current?.cared_name ? `Próximos passos\npara ${current.cared_name}` : 'Combinar\npróximos passos'} />
    ...
    {linksLoading && links.length === 0 ? <Section top={0}><Card radius={24} padding={20}><Txt s={13} c={T.t3}>Carregando conexões…</Txt></Card></Section> : linksError ? <Section top={0}><CareLinksErrorCard error={linksError} onRetry={refreshLinks} /></Section> : !current ? <Section top={0}><Card radius={24} padding={20}><Txt s={14.5} w={800} c={T.t1}>Conecte uma pessoa para criar um plano.</Txt></Card></Section> : current.consentConfigurationInactive || configurationInactive ? <Section top={0}><CareConfigurationCard feature="O plano, a rede de apoio e as notas privadas" /></Section> : current.consent?.status !== 'active' ? <Section top={0}><Card radius={24} padding={20}><Txt s={14.5} w={800} c={T.t1}>Plano bloqueado até haver consentimento</Txt><Txt s={12.5} lh={1.5} c={T.t3} style={{ marginTop: 6 }}>A pessoa acompanhada controla se o plano pode ser compartilhado.</Txt></Card></Section> : <>
